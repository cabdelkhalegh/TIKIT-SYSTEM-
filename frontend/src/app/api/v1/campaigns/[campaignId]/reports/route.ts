...
// GET /api/v1/campaigns/:campaignId/reports — list reports
export async function GET(req: NextRequest, { params }: { params: { campaignId: string } }) {
  try {
    const url = `${BACKEND_URL}/api/v1/campaigns/${params.campaignId}/reports`;
...
// POST /api/v1/campaigns/:campaignId/reports — create report
export async function POST(req: NextRequest, { params }: { params: { campaignId: string } }) {
  try {
    const body = await req.json();
    const response = await fetch(
      `${BACKEND_URL}/api/v1/campaigns/${params.campaignId}/reports`,
