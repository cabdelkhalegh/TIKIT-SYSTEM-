...
// POST /api/v1/campaigns/:campaignId/content/:id/live-url
export async function POST(req: NextRequest, { params }: { params: { campaignId: string; id: string } }) {
  try {
    const body = await req.json();
    const response = await fetch(
      `${BACKEND_URL}/api/v1/content/${params.id}/live-url`,
      { method: 'POST', headers: getHeaders(req), body: JSON.stringify(body) }
    );
    const data = await response.json();
    return NextResponse.json(data, { status: response.status });
  } catch (error: any) {
    console.error('Error proxying POST content live-url:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to submit live URL' },
      { status: 500 }
    );
  }
}
